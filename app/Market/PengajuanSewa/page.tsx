...
import PengajuanSewaCard from '@/components/template/PengajuanSewaCard'
import SectionHeader from '@/components/template/SectionHeader'
import { PENGAJUAN_SEWA, SIDEBAR_LIST } from '@/constant'
    ...
    <>
      ...
      <section className='flex justify-center px-5 pt-8 pb-20 gap-5'>
        ...
        <div className='w-[70%] border rounded-tl-lg rounded-b-lg shadow-lg'>
          {/* Header */}
          <SectionHeader title='Pengajuan Sewa' placeholder='Cari Pengajuan' buttonVisibility='hidden' name='pengajuan' link='/Market/Vehicle/AddVehicle'/>
          {/* End of Header */}

          <div className='flex flex-col gap-5 p-6'>
            {PENGAJUAN_SEWA.map((pengajuan) => (
              <PengajuanSewaCard key={pengajuan.sewaID} sewaID={pengajuan.sewaID} penyewa={pengajuan.penyewa} lamaSewa={pengajuan.lamaSewa} mulaiSewa={pengajuan.mulaiSewa} selesaiSewa={pengajuan.selesaiSewa} total={pengajuan.total} img={pengajuan.img} kendaraan={pengajuan.kendaraan} />
            ))}
          </div>
        </div>
      </section>
